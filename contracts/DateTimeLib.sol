...
library DateTimeLib {
        ...
        function leapYearsBefore(uint year) public  returns (uint) {
                year -= 1;
                return year / 4 - year / 100 + year / 400;
        }

        function getDaysInMonth(uint8 month, uint16 year) public  returns (uint8) {
        ...
        function parseTimestamp(uint timestamp) internal  returns (_DateTime dt) {
        ...
        function getMonth(uint timestamp) returns (uint8) {
